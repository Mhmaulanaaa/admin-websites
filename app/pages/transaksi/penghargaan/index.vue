...
import BaseModalTransaksiPenghargaan from "~/components/base/transaksi/BaseModalTransaksiPenghargaan.vue";

useHead({
  title: "Admin Transaksi Penghargaan",
});
definePageMeta({
  breadcrumb: [{ label: "Transaksi", to: "/transaksi" }, { label: "Penghargaan" }],
});
...
const dataTable = ref([
  {
    kode_penghargaan: "PNG20260429001",
  ...
  {
    kode_penghargaan: "PNG20260429002",
...
const kodeOptions = computed(() =>
  dataTable.value.map((i) => ({
    label: `${i.kode_penghargaan} - ${i.nama_penghargaan}`,
    value: i.kode_penghargaan,
  }))
);
...
const filteredData = computed(() => {
  return dataTable.value.filter((item) => {
    const matchKode =
      !appliedFilter.value.kode || item.kode_penghargaan === appliedFilter.value.kode;
...
const columns = [
  ...
  { accessorKey: "kode_penghargaan", header: "Kode Penghargaan" },
  { accessorKey: "nama_penghargaan", header: "Nama Penghargaan" },
...
<template>
  ...
  <div class="grid grid-cols-3 gap-4 mb-6">
    ...
    <BaseStatCard
      title="Total Penghargaan Tidak Aktif"
  ...
  >
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
      ...
      <div class="w-full group flex items-center sm:w-full group gap-5">
        <div class="flex-1">
          <USelectMenu
            label="Pilih Nomor dan Nama Penghargaan"
            placeholder="Pilih Nomor dan Nama Penghargaan"
            v-model="kodeFilter"
            :items="kodeOptions"
            value-key="value"
            label-key="label"
  ...
  >
    ...
    >
      ...
      <!-- KODE PENGHARGAAN -->
      <template #kode_penghargaan-cell="{ row }">
        <span class="text-xs font-medium text-gray-700 dark:text-slate-200">
          {{ (row.original as Menu).kode_penghargaan }}
        </span>
      </template>

      <!-- NAMA PENGHARGAAN -->
      <template #nama_penghargaan-cell="{ row }">
        <span class="text-xs">
          {{ (row.original as Menu).nama_penghargaan }}
        </span>
      </template>

      <!-- INSTANSI -->
      <template #instansi-cell="{ row }">
        <span class="text-xs">
          {{ (row.original as Menu).instansi }}
        </span>
      </template>
