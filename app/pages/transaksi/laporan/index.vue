...
import BaseModalTransaksiLaporan from "~/components/base/transaksi/BaseModalTransaksiLaporan.vue";

useHead({
  title: "Admin Transaksi Laporan",
});
definePageMeta({
  breadcrumb: [{ label: "Transaksi", to: "/transaksi" }, { label: "Laporan" }],
});
...
const dataTable = ref([
  {
    kode_laporan: "LP20260429001",
    judul: "Laporan 1",
    deskripsi: "Deskripsi singkat untuk laporan 1",
    tanggal: "2024-04-29",
    status: "aktif",
  },
  {
    kode_laporan: "LP20260429002",
    judul: "Laporan 2",
    deskripsi: "Deskripsi singkat untuk laporan 2",
    tanggal: "2024-04-29",
    status: "tidak_aktif",
  },
]);
...
const kodeOptions = computed(() =>
  dataTable.value.map((i) => ({
    label: `${i.kode_laporan} - ${i.judul}`,
    value: i.kode_laporan,
  }))
);
...
const filteredData = computed(() => {
  return dataTable.value.filter((item) => {
    const matchKode =
      !appliedFilter.value.kode || item.kode_laporan === appliedFilter.value.kode;
...
const columns = [
  ...
  { accessorKey: "kode_laporan", header: "Kode Laporan" },
...
<template>
  ...
  <div class="grid grid-cols-3 gap-4 mb-6">
    ...
    <BaseStatCard
      title="Total Laporan Tidak Aktif"
  ...
  >
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
      ...
      <div class="w-full group flex items-center sm:w-full group gap-5">
        <div class="flex-1">
          <USelectMenu
            label="Pilih Nomor dan Nama Laporan"
            placeholder="Pilih Nomor dan Nama Laporan"
            v-model="kodeFilter"
            :items="kodeOptions"
            value-key="value"
            label-key="label"
