...
import BaseModalTransaksiKerjasamaPembiayaan from "~/components/base/transaksi/BaseModalTransaksiKerjasamaPembiayaan.vue";

useHead({
  title: "Admin Transaksi Kerjasama Pembiayaan",
});
definePageMeta({
  breadcrumb: [
    { label: "Transaksi", to: "/transaksi" },
    { label: "Kerjasama Pembiayaan" },
  ],
});
...
const dataTable = ref([
  {
    kode_kerjasama: "KJP20260429001",
    nama_pembiayaan: "Nama Pembiayaan 1",
    file: "https://example.com/file1.jpg",
    status: "aktif",
  },
  {
    kode_kerjasama: "KJP20260429002",
    nama_pembiayaan: "Nama Pembiayaan 2",
    file: "https://example.com/file1.jpg",
    status: "tidak_aktif",
  },
]);
...
const kodeOptions = computed(() =>
  dataTable.value.map((i) => ({
    label: `${i.kode_kerjasama} - ${i.nama_pembiayaan}`,
    value: i.kode_kerjasama,
  }))
);
...
const filteredData = computed(() => {
  return dataTable.value.filter((item) => {
    const matchKode =
      !appliedFilter.value.kode || item.kode_kerjasama === appliedFilter.value.kode;
...
const columns = [
  ...
  { accessorKey: "kode_kerjasama", header: "Kode Kerjasama" },
  { accessorKey: "nama_pembiayaan", header: "Nama Pembiayaan" },
...
<template>
  ...
  <div class="grid grid-cols-3 gap-4 mb-6">
    <BaseStatCard
      title="Total Kerjasama Pembiayaan"
    ...
    <BaseStatCard
      title="Total Kerjasama Pembiayaan Aktif"
    ...
    <BaseStatCard
      title="Total Kerjasama Pembiayaan Tidak Aktif"
  ...
  >
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
      ...
      <div class="w-full group flex items-center sm:w-full group gap-5">
        <div class="flex-1">
          <USelectMenu
            label="Pilih Nomor dan Nama Kerjasama Pembiayaan"
            placeholder="Pilih Nomor dan Nama Kerjasama Pembiayaan"
            v-model="kodeFilter"
            :items="kodeOptions"
            value-key="value"
            label-key="label"
  ...
  >
    ...
    >
      ...
      <!-- KODE KERJASAMA -->
      <template #kode_kerjasama-cell="{ row }">
        <span class="text-xs font-medium text-gray-700 dark:text-slate-200">
          {{ (row.original as Menu).kode_kerjasama }}
        </span>
      </template>

      <!-- NAMA PEMBIAYAAN -->
      <template #nama_pembiayaan-cell="{ row }">
        <span class="text-xs">
          {{ (row.original as Menu).nama_pembiayaan }}
        </span>
      </template>
    ...
    <!-- End Table -->
    <div>
      <BaseModalTransaksiKerjasamaPembiayaan
        size="lg"
