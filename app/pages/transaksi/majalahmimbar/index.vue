...
import BaseModalTransaksiMajalahMimbar from "~/components/base/transaksi/BaseModalTransaksiMajalahMimbar.vue";

useHead({
  title: "Admin Transaksi Majalah Mimbar",
});
definePageMeta({
  breadcrumb: [{ label: "Transaksi", to: "/transaksi" }, { label: "Majalah Mimbar" }],
});
...
const dataTable = ref([
  {
    kode_majalah: "MM20260429001",
    judul: "Majalah 1",
    deskripsi: "Deskripsi singkat untuk majalah 1",
    tanggal: "2024-04-29",
    status: "aktif",
  },
  {
    kode_majalah: "MM20260429002",
    judul: "Majalah 2",
    deskripsi: "Deskripsi singkat untuk majalah 2",
    tanggal: "2024-04-29",
    status: "tidak_aktif",
  },
]);
...
const kodeOptions = computed(() =>
  dataTable.value.map((i) => ({
    label: `${i.kode_majalah} - ${i.judul}`,
    value: i.kode_majalah,
  }))
);
...
const filteredData = computed(() => {
  return dataTable.value.filter((item) => {
    const matchKode =
      !appliedFilter.value.kode || item.kode_majalah === appliedFilter.value.kode;
...
const columns = [
  ...
  { accessorKey: "kode_majalah", header: "Kode Majalah" },
...
<template>
  ...
  <div class="mb-4">
    <h1 class="text-xl font-semibold text-gray-800 dark:text-white mb-1">
      Transaksi Majalah Mimbar
    </h1>
    <AppBreadcrumb />
  </div>
  <div class="grid grid-cols-3 gap-4 mb-6">
    <BaseStatCard
      title="Total Majalah Mimbar"
    ...
    <BaseStatCard
      title="Total Majalah Mimbar Aktif"
    ...
    <BaseStatCard
      title="Total Majalah Mimbar Tidak Aktif"
  ...
  >
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
      ...
      <div class="w-full group flex items-center sm:w-full group gap-5">
        <div class="flex-1">
          <USelectMenu
            label="Pilih Nomor dan Nama Majalah Mimbar"
            placeholder="Pilih Nomor dan Nama Majalah Mimbar"
            v-model="kodeFilter"
            :items="kodeOptions"
            value-key="value"
            label-key="label"
  ...
  >
    ...
    <!-- End Table -->
    <div>
      <BaseModalTransaksiMajalahMimbar
        size="lg"
