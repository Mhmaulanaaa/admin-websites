...
import BaseModalTransaksiHariPeringatan from "~/components/base/transaksi/BaseModalTransaksiHariPeringatan.vue";

useHead({
  title: "Admin Transaksi Hari Peringatan",
});
definePageMeta({
  breadcrumb: [{ label: "Transaksi", to: "/transaksi" }, { label: "Hari Peringatan" }],
});
...
const dataTable = ref([
  {
    kode_hari_peringatan: "HP20260429001",
    nama_hari_peringatan: "Judul Hari Peringatan 1",
    tanggal: "2022-12-30",
    file: "https://example.com/file1.jpg",
    status: "aktif",
  },
  {
    kode_hari_peringatan: "HP20260429002",
    nama_hari_peringatan: "Judul Hari Peringatan 2",
    tanggal: "2024-03-05",
    file: "https://example.com/file1.jpg",
    status: "tidak_aktif",
  },
...
const kodeOptions = computed(() =>
  dataTable.value.map((i) => ({
    label: `${i.kode_hari_peringatan} - ${i.nama_hari_peringatan}`,
    value: i.kode_hari_peringatan,
  }))
);
...
const filteredData = computed(() => {
  return dataTable.value.filter((item) => {
    const matchKode =
      !appliedFilter.value.kode || item.kode_hari_peringatan === appliedFilter.value.kode;
...
interface Menu {
  kode_hari_peringatan: string;
  nama_hari_peringatan: string;
  file: string;
  tanggal: string;
  status: string;
}
...
const columns = [
  ...
  { accessorKey: "kode_hari_peringatan", header: "Kode Hari Peringatan" },
  { accessorKey: "nama_hari_peringatan", header: "Nama Hari Peringatan" },
...
<template>
  ...
  <div class="mb-4">
    <h1 class="text-xl font-semibold text-gray-800 dark:text-white mb-1">
      Transaksi Hari Peringatan
    </h1>
    <AppBreadcrumb />
  </div>
  <div class="grid grid-cols-3 gap-4 mb-6">
    <BaseStatCard
      title="Total Hari Peringatan"
    ...
    <BaseStatCard
      title="Total Hari Peringatan Aktif"
    ...
    <BaseStatCard
      title="Total Hari Peringatan Tidak Aktif"
  ...
  >
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
      ...
      <div class="w-full group flex items-center sm:w-full group gap-5">
        <div class="flex-1">
          <USelectMenu
            label="Pilih Nomor dan Nama Hari Peringatan"
            placeholder="Pilih Nomor dan Nama Hari Peringatan"
            v-model="kodeFilter"
            :items="kodeOptions"
            value-key="value"
            label-key="label"
  ...
  >
    ...
    >
      ...
      <!-- KODE HARI PERINGATAN -->
      <template #kode_hari_peringatan-cell="{ row }">
        <span class="text-xs font-medium text-gray-700 dark:text-slate-200">
          {{ (row.original as Menu).kode_hari_peringatan }}
        </span>
      </template>

      <!-- NAMA HARI PERINGATAN -->
      <template #nama_hari_peringatan-cell="{ row }">
        <span class="text-xs">
          {{ (row.original as Menu).nama_hari_peringatan }}
        </span>
      </template>
    ...
    <!-- End Table -->
    <div>
      <BaseModalTransaksiHariPeringatan
        size="lg"
