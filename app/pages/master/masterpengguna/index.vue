...
useHead({
  title: "Admin Master Pengguna",
});
definePageMeta({
  breadcrumb: [{ label: "Master", to: "/master" }, { label: "Master Pengguna" }],
});
...
const kodeOptions = computed(() =>
  dataTable.value.map((i) => ({
    label: `${i.kode_pengguna} - ${i.nama_pengguna}`,
    value: i.kode_pengguna,
  }))
);

const hakaksesOptions = computed(() =>
  [...new Set(dataTable.value.map((i) => i.hak_akses))].map((v) => ({
    label: v,
    value: v,
  }))
);
...
function handleSearch() {
  appliedFilter.value = {
    ...
    hak_akses: hakaksesFilter.value,
...
function handleReset() {
  ...
  hakaksesFilter.value = undefined;
...
const filteredData = computed(() => {
  return dataTable.value.filter((item) => {
    const matchKode =
      !appliedFilter.value.kode || item.kode_pengguna === appliedFilter.value.kode;

    const matchHakAkses =
      !appliedFilter.value.hak_akses || item.hak_akses === appliedFilter.value.hak_akses;
    ...
    return matchKode && matchHakAkses && matchUnitKerja && matchStatus;
  });
});
...
const columns = [
  ...
  { accessorKey: "kode_pengguna", header: "Kode Pengguna" },
  { accessorKey: "nama_pengguna", header: "Nama Pengguna" },
...
<template>
  ...
  >
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
      ...
      <div class="w-full group flex items-center sm:w-full group gap-5">
        <div class="flex-1">
          <USelectMenu
            label="Pilih Kode dan Nama Pengguna"
            placeholder="Pilih Kode dan Nama Pengguna"
            v-model="kodeFilter"
            :items="kodeOptions"
            value-key="value"
            label-key="label"
  ...
  >
    ...
    >
      ...
      <!-- KODE PENGGUNA -->
      <template #kode_pengguna-cell="{ row }">
        <span class="text-xs font-medium text-gray-700 dark:text-slate-200">
          {{ (row.original as Menu).kode_pengguna }}
        </span>
      </template>

      <!-- NAMA PENGGUNA -->
      <template #nama_pengguna-cell="{ row }">
        <span class="text-xs">
          {{ (row.original as Menu).nama_pengguna }}
        </span>
      </template>

      <!-- HAK AKSES -->
      <template #hak_akses-cell="{ row }">
        <span class="text-xs">
          {{ (row.original as Menu).hak_akses }}
        </span>
      </template>
