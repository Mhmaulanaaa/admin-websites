...
import BaseModalMasterSKTT from "~/components/base/master/BaseModalMasterSKTT.vue";

useHead({
  title: "Admin Master SK TT",
});
definePageMeta({
  breadcrumb: [{ label: "Master", to: "/master" }, { label: "Master SK TT" }],
});
...
const dataTable = ref([
  {
    kode_mastersk: "MSKTT20260429001",
    nama_mastersk: "Nama Master SK TT 1",
    file: "https://example.com/file1.pdf",
    status: "aktif",
  },
  {
    kode_mastersk: "MSKTT20260429002",
    nama_mastersk: "Nama Master SK TT 2",
    file: "https://example.com/file1.pdf",
    status: "tidak_aktif",
  },
]);
...
const kodeOptions = computed(() =>
  dataTable.value.map((i) => ({
    label: `${i.kode_mastersk} - ${i.nama_mastersk}`,
    value: i.kode_mastersk,
  }))
);
...
const filteredData = computed(() => {
  return dataTable.value.filter((item) => {
    const matchKode =
      !appliedFilter.value.kode || item.kode_mastersk === appliedFilter.value.kode;
...
const columns = [
  ...
  { accessorKey: "kode_mastersk", header: "Kode Master SK TT" },
  { accessorKey: "nama_mastersk", header: "Nama Master SK TT" },
...
<template>
  <!-- Start Section -->
  <div class="mb-4">
    <h1 class="text-xl font-semibold text-gray-800 dark:text-white mb-1">Master SK TT</h1>
    <AppBreadcrumb />
  </div>
  <div class="grid grid-cols-3 gap-4 mb-6">
    <BaseStatCard
      title="Total Master Sk TT"
    ...
    <BaseStatCard
      title="Total Master Sk TT Aktif"
    ...
    <BaseStatCard
      title="Total Master Sk TT Tidak Aktif"
  ...
  >
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
      ...
      <div class="w-full group flex items-center sm:w-full group gap-5">
        <div class="flex-1">
          <USelectMenu
            label="Pilih Kode dan Nama SK TT"
            placeholder="Pilih Kode dan Nama SK TT"
            v-model="kodeFilter"
            :items="kodeOptions"
            value-key="value"
            label-key="label"
  ...
  >
    ...
    >
      ...
      <!-- KODE MASTER SK TT -->
      <template #kode_mastersk-cell="{ row }">
        <span class="text-xs font-medium text-gray-700 dark:text-slate-200">
          {{ (row.original as Menu).kode_mastersk }}
        </span>
      </template>

      <!-- NAMA MASTER SK TT -->
      <template #nama_mastersk-cell="{ row }">
        <span class="text-xs">
          {{ (row.original as Menu).nama_mastersk }}
        </span>
      </template>
