...
function generateKode() {
  ...
  form.kode_survei = `SJT${tanggal}${urutan}`;
}
...
watch(
  ...
  (val) => {
    ...
    if (props.isEdit && props.initialData) {
      ...
      form.kode_survei = initialData.kode_survei;
    ...
    <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="text-sm font-semibold text-slate-800 dark:text-white">
                {{ isEdit ? "Edit Survei Website" : "Tambah Survei Website" }}
              </h2>
              <p class="text-xs text-slate-500">
                {{
                  isEdit
                    ? "Perbarui data survei website"
                    : "Tambahkan survei website baru"
                }}
              </p>
            </div>
          </div>
        ...
        <div class="p-6 m-1 gap-2">
          <div v-if="isEdit">
            <label class="text-sm">Kode Survei Website</label>
            <BaseInput
              v-model="form.kode_survei"
              placeholder="Kode Survei Website"
              disabled
            />
          </div>
          <div>
