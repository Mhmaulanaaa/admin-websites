...
const form = reactive({
  kode_hari_peringatan: "",
  nama_hari_peringatan: "",
  file: "" as string,
  tanggal: "",
  status: "aktif",
});
...
function generateKode() {
  ...
  form.kode_hari_peringatan = `HP${tanggal}${urutan}`;
}
...
watch(
  ...
  (val) => {
    ...
    if (props.isEdit && props.initialData) {
      ...
      form.kode_hari_peringatan = initialData.kode_hari_peringatan;

      form.nama_hari_peringatan = initialData.nama_hari_peringatan;
      form.file = initialData.file ?? "";
      form.tanggal = initialData.tanggal;
      form.status = initialData.status;
    } else {
      generateKode();
      form.nama_hari_peringatan = "";
      form.file = "";
      form.tanggal = "";
      form.status = "aktif";
    }
    ...
    <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="text-sm font-semibold text-slate-800 dark:text-white">
                {{ isEdit ? "Edit Hari Peringatan" : "Tambah Hari Peringatan" }}
              </h2>
              <p class="text-xs text-slate-500">
                {{ isEdit ? "Perbarui data hari peringatan" : "Tambahkan hari peringatan baru" }}
              </p>
            </div>
          </div>
        ...
        <div class="p-6 m-1 gap-2">
          <div v-if="isEdit">
            <label class="text-sm">Kode Hari Peringatan</label>
            <BaseInput v-model="form.kode_hari_peringatan" placeholder="Kode Hari Peringatan" disabled />
          </div>
          <div>
            <label class="text-sm">Nama Hari Peringatan</label>
            <BaseInput v-model="form.nama_hari_peringatan" placeholder="Nama Hari Peringatan" />
          </div>
          <div>
