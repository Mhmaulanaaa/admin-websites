...
function generateKode() {
  ...
  form.kode_profil = `PRD${tanggal}${urutan}`;
}
...
watch(
  ...
  (val) => {
    ...
    if (props.isEdit && props.initialData) {
      ...
      form.kode_profil = initialData.kode_profil;

      form.nama_profil = initialData.nama_profil;
      form.nama_jabatan = initialData.nama_jabatan;
    ...
    <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="text-sm font-semibold text-slate-800 dark:text-white">
                {{ isEdit ? "Edit Profil Direksi" : "Tambah Profil Direksi" }}
              </h2>
              <p class="text-xs text-slate-500">
                {{
                  isEdit
                    ? "Perbarui data profil direksi"
                    : "Tambahkan profil direksi baru"
                }}
              </p>
            </div>
          </div>
        ...
        <div class="p-6 m-1 gap-2">
          <div v-if="isEdit">
            <label class="text-sm">Kode Profil</label>
            <BaseInput v-model="form.kode_profil" placeholder="Kode Profil" disabled />
          </div>
          <div>
            <label class="text-sm">Nama Profil</label>
            <BaseInput v-model="form.nama_profil" placeholder="Nama Profil" />
          </div>
          <div>
            <label class="text-sm">Nama Jabatan</label>
            <BaseInput v-model="form.nama_jabatan" placeholder="Nama Jabatan" />
          </div>
          <div>
