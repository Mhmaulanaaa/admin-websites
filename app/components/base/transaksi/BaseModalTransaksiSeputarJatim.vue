...
function generateKode() {
  ...
  form.kode_seputarjatim = `SJT${tanggal}${urutan}`;
}
...
watch(
  ...
  (val) => {
    ...
    if (props.isEdit && props.initialData) {
      ...
      form.kode_seputarjatim = initialData.kode_seputarjatim;
    ...
    <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="text-sm font-semibold text-slate-800 dark:text-white">
                {{ isEdit ? "Edit Seputar Jatim" : "Tambah Seputar Jatim" }}
              </h2>
              <p class="text-xs text-slate-500">
                {{
                  isEdit ? "Perbarui data seputar jatim" : "Tambahkan seputar jatim baru"
                }}
              </p>
            </div>
          </div>
        ...
        <div class="p-6 m-1 gap-2">
          <div v-if="isEdit">
            <label class="text-sm">Kode Seputar Jatim</label>
            <BaseInput
              v-model="form.kode_seputarjatim"
              placeholder="Kode Seputar Jatim"
              disabled
            />
          </div>
          <div>
