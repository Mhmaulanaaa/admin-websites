...
function generateKode() {
  ...
  form.kode_penghargaan = `PNG${tanggal}${urutan}`;
}
...
watch(
  ...
  (val) => {
    ...
    if (props.isEdit && props.initialData) {
      ...
      form.kode_penghargaan = initialData.kode_penghargaan;

      form.nama_penghargaan = initialData.nama_penghargaan;
      form.instansi = initialData.instansi;
      form.file = initialData.file ?? "";
      form.tanggal = initialData.tanggal;
      form.status = initialData.status;
    } else {
      generateKode();
      form.nama_penghargaan = "";
      form.instansi = "";
      form.file = "";
      form.tanggal = "";
      form.status = "aktif";
    ...
    <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
        ...
        <div class="p-6 m-1 gap-2">
          <div v-if="isEdit">
            <label class="text-sm">Kode Penghargaan</label>
            <BaseInput
              v-model="form.kode_penghargaan"
              placeholder="Kode Penghargaan"
              disabled
            />
          </div>
          <div>
            <label class="text-sm">Nama Penghargaan</label>
            <BaseInput v-model="form.nama_penghargaan" placeholder="Nama Penghargaan" />
          </div>
          <div>
            <label class="text-sm">Instansi</label>
            <BaseInput v-model="form.instansi" placeholder="Instansi" />
          </div>
          <div>
