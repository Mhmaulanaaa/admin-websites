...
function generateKode() {
  ...
  form.kode_kerjasama = `KJP${tanggal}${urutan}`;
}
...
watch(
  ...
  (val) => {
    ...
    if (props.isEdit && props.initialData) {
      ...
      form.kode_kerjasama = initialData.kode_kerjasama;

      form.nama_pembiayaan = initialData.nama_pembiayaan;
      form.file = initialData.file ?? "";
      form.status = initialData.status;
    } else {
      generateKode();
      form.nama_pembiayaan = "";
      form.file = "";
      form.status = "aktif";
    }
  },
    ...
    <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      >
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="text-sm font-semibold text-slate-800 dark:text-white">
                {{ isEdit ? "Edit Kerjasama Pembiayaan" : "Tambah Kerjasama Pembiayaan" }}
              </h2>
              <p class="text-xs text-slate-500">
                {{
                  isEdit
                    ? "Perbarui data kerjasama pembiayaan"
                    : "Tambahkan kerjasama pembiayaan baru"
                }}
              </p>
            </div>
          </div>
        ...
        <div class="p-6 m-1 gap-2">
          <div v-if="isEdit">
            <label class="text-sm">Kode Kerjasama</label>
            <BaseInput
              v-model="form.kode_kerjasama"
              placeholder="Kode Kerjasama"
              disabled
            />
          </div>
          <div>
            <label class="text-sm">Nama Pembiayaan</label>
            <BaseInput v-model="form.nama_pembiayaan" placeholder="Nama Pembiayaan" />
          </div>
          <div>
